...
const schema = zod.object({
  ...
  okpo: zod.string().optional(),
  payment_details: zod.string().optional(),
});
...
const InvoiceConfigPage = () => {
  ...
  const getFormDefaultValues = useCallback(() => {
    return {
      ...
      vat_number: data?.invoice_config.vat_number || "",
      okpo: data?.invoice_config.okpo || "",
      payment_details: data?.invoice_config.payment_details || "",
    };
  }, [data]);
  ...
  return (
    <Container className="divide-y p-0">
      ...
      <FormProvider {...form}>
        <form onSubmit={handleSubmit} className="flex h-full flex-col overflow-hidden p-2 gap-2 w-1/2">
          <Controller
            ...
            render={({ field }) => {
              return (
                <div className="flex flex-col space-y-2">
                  ...
                  {field.value && <img src={field.value} alt="Company Logo" className="mt-2 h-24 w-24" />}
                </div>
              );
            }}
          />
          <Button className="mt-8" type="submit" disabled={isLoading || isPending}>
            Save
          </Button>
        </form>
      </FormProvider>
