...
export const generateInvoicePdfWorkflow = createWorkflow("generate-invoice-pdf", (input: WorkflowInput) => {
  // @ts-ignore
  ...
  const { data: company } = useQueryGraphStep({
    entity: "company",
    fields: ["*"],
    filters: {
      name: companyName,
  ...
  }).config({ name: "retrieve-companies" });
    ...
    (data) => {
      ...
      if (order.billing_address?.country_code) {
        order.billing_address.country_code =
          data.countries.find((country) => country.iso_2 === order.billing_address!.country_code)?.display_name ||
          order.billing_address!.country_code;
      }

      if (order.shipping_address?.country_code) {
        order.shipping_address.country_code =
          data.countries.find((country) => country.iso_2 === order.shipping_address!.country_code)?.display_name ||
          order.shipping_address!.country_code;
      }
